...
Here $T$ is the temperature [$^\circ C$], $I$ is the daily irradiance [M quanta m$^{-2}$ d$^{-1}$], and $\tau$ is the nutrients-limited growth rate [d$^{-1}$]. $\tau = \tau_{N} \tau_{Si} \tau_{PO_{4}^{3-}}$ where $\tau_{N}$ is the nitrogen-limited growth rate, $\tau_{Si}$ is he silicon-limited growth rate, and $\tau_{P}$ is the phosphorus-limited growth rate: $\tau_{N} = \tau_{NH_{4}^{+}} + \tau_{NO_{x}}$, $\tau_{NH_{4}^{+}} = f_{r}(K_{NH_{4}^{+}}, [$NH$_{4}^{+}])$, $\tau_{NO_{x}} = f_{r}(K_{NO_{x}}, [$NO$_{2}^{-}]+[$NO$_{3}^{-}])$ $\cdot$ $f_{i}(K_{NH_{4}^{+}}, [$NH$_{4}^{+}])$; $\tau_{Si} = f_{r}(K_{Si}, [$Si$])$; $\tau_{PO_{4}^{3-}} = f_{r}(K_{PO_{4}^{3-}}, [$PO$_{4}^{3-}])$. $f_r$ and $f_i$ are the sigmoid type functional response and inhibitor functions respectively of the following forms: $f_r(K_R, R) = \frac{R^2}{K_R^2 + R^2}$ and $f_i(K_R, R) = \frac{K_R^2}{K_R^2 + R^2}$ where R is the limiting or inhibiting resource concentration and $K_R$ is the corresponding half-saturation constant. Therefore for an instance in case of high availability of ammonium ($[$NH$_{4}^{+}] >> K_{NH_{4}^{+}}$) $\tau_{NH_{4}^{+}}$ tends to one while the nitrates+nitrites limiting term $\tau_{NO_{x}}$ inhibited by ammonium tends to zero.
