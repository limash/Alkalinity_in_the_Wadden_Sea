...
def ChlCratio(temperature, irradiance, nutrient_limiter):
    ...
    A, B, C = (0.0154, 0.050, 0.059)  # achieved by experiment
...
def phy_daily_growth_rate(depth, k, pbm, alpha, nutrient_limiter,
    ...
    ChlCratio_d = ChlCratio(temperature_d,
                            light_attenuation(k, depth, irradiance_d),
                            nutrient_limiter)
    phy_biorate_d = phy_biorate(photoperiod_d, pbm, alpha,
                                light_attenuation(k, depth, par_d))
...
def zoo_phy_graz(k_het_phy_gro, k_het_phy_lim, het, phy):
    ...
    return k_het_phy_gro*het*sigmoid_powered_limiter(k_het_phy_lim,
                                                     quota(phy, het), 2)


def zoo_pom_graz(k_het_pom_gro, k_het_pom_lim, het, pom):
    """Grazing of zoo on detritus"""
    return k_het_pom_gro*het*sigmoid_powered_limiter(k_het_pom_lim,
                                                     quota(pom, het), 2)
...
def n2_fixation(k_nfix, lim_p, nh4, no2, no3, po4, growth_phy):
    return (k_nfix*lim_p*1/(1+np.power(((no3+no2+nh4)/n_zero(po4)*16), 4))
            * growth_phy)
...
def calculate(depth, k, k_mix, latitude, days, temperature,
    ...
    for day in np.nditer(days):
        ...
        for circle in np.nditer(circles):
            ...
            graz_pom = (zoo_pom_graz(k_het_pom_gro, k_het_pom_lim,
                                     inhet[circle], inpom[circle])
                        / number_of_circles)
            ...
            # N2 mM N/m^3
            n_fixation = 0
            n_nitrif_1 = (nitrification1(k_nitrif1, o2s_nf,
                                         innh4[circle], ino2[circle])
                          / number_of_circles)
            n_nitrif_2 = (nitrification2(k_nitrif2, o2s_nf,
                                         inno2[circle], ino2[circle])
                          / number_of_circles)
            ...
            autolysis_l = autolysis_labile(k_pom_dom,
                                           inpom[circle]) / number_of_circles
            # oxidation of om
            kf = om_oxo2_coef(k_omox_o2, ino2[circle], temperature[day], tref)
            if (ino2[circle] <
                (carbon_g_to_mole(k_pom_ox*inpom[circle]*kf)
                 / number_of_circles
                 + carbon_g_to_mole(k_dom_ox*indom[circle]*kf)
                 / number_of_circles)):
                kf = 0
            ...
            # increments
            dnh4_bio = (-dphy_in_m/phy_c_to_n*quota(nh4_limiter,
                                                    nitrogen_limiter)
            ...
            dno2_bio = (-dphy_in_m/phy_c_to_n*quota(no3_limiter,
                                                    nitrogen_limiter)
            ...
            dno3_bio = (-dphy_in_m/phy_c_to_n*quota(no3_limiter,
                                                    nitrogen_limiter)
            ...
            # check nutrients for negative values and in case return values
            foo = [innh4[circle+1] < 0, inno2[circle+1] < 0,
                   inno3[circle+1] < 0,
        ...
        chl_a[day+1] = phy[day+1]*ChlCratio(temperature[day+1],
                                            irradiance[day+1],
                                            nutrient_limiter)
...
def c_from_ratio(chl_a, temperature, k, I, depth,
    ...
    return (chl_a
            / ChlCratio(temperature, light_attenuation(k, depth, I),
                        nutrient_limiter))
